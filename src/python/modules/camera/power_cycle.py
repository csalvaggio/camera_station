...
def power_cycle(station_parameters,
                shutdown_duration=15,
                startup_duration=15,
                verbose=False):
   # Raspberry Pi camera
   if station_parameters['cameraType'].lower() == 'rpi':
      camera.power_cycle_rpi_camera(shutdown_duration=shutdown_duration,
                                    startup_duration=startup_duration,
                                    verbose=verbose)
   # gPhoto2 camera
   elif station_parameters['cameraType'].lower() == 'gphoto2':
      camera.power_cycle_gphoto2_camera(shutdown_duration=shutdown_duration,
                                        startup_duration=startup_duration,
                                        verbose=verbose)
   else:
